...
async function run() {
  const engine = await InferenceSession.create('packages/services/models/u2net-portrait-matting.onnx', {
  ...
  console.log('Matting range:', low, high);

  // Check shadow pixel
  const x = Math.floor(size * 0.68);
  const y = Math.floor(size * 0.35);
  console.log('Matting shadow pixel value:', prediction[y * size + x]);
  ...
  fs.writeFileSync('artifacts/ai-review/test_portrait_matting.png', resBuffer);
  console.log('Saved artifacts/ai-review/test_portrait_matting.png');
}
